...
public:
    ...
    __forceinline Vec2f MousePosNormalizedToWindow() const
    {
        return Vec2f(float(_Mouse.Pos.x) / float(_WindowDimensions.x), float(_Mouse.Pos.y) / float(_WindowDimensions.y));
    }
