...
void BaseMesh::LoadGridPly(const String &Filename)
{
        ...
        if(CurLine.Length() > 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    IndexGrid.GetElement(CurGridRow, CurGridCol++) = Index;
    ...
    for(UINT Row = 0; Row < IndexGrid.Rows() - 1; Row++)
    {
        for(UINT Col = 0; Col < IndexGrid.Cols() - 1; Col++)
        {
            ...
            I[0] = IndexGrid.GetElement(Row + 0, Col + 0);
            I[1] = IndexGrid.GetElement(Row + 1, Col + 0);
            I[2] = IndexGrid.GetElement(Row + 1, Col + 1);
            I[3] = IndexGrid.GetElement(Row + 0, Col + 1);
...
void BaseMesh::LoadPly(const String &Filename)
{
    ...
    while(!File.eof())
    {
        ...
        if(!HeaderDone)
        {
            if(CurLine.IsPrefix(String("end_header")))
            {
                HeaderDone = true;
            }
        }
...
void BaseMesh::SaveBinary(const String &Filename)
{
    String FilenameCopy = Filename;
    FILE *file;
    file = fopen(FilenameCopy.CString(), "wb");
    
    int vc=VertexCount(),ic=IndexCount();
    MeshVertex *V = Vertices();
    DWORD *I = Indices();

    fwrite(&vc, sizeof(int), 1, file);
    fwrite(&ic, sizeof(int), 1, file);
    fwrite(V, sizeof(MeshVertex), vc, file);
    fwrite(I, sizeof(DWORD), ic, file);
    fclose(file);
}
...
void BaseMesh::LoadBinary(const String &Filename)
{
    String FilenameCopy = Filename;
    FILE *file;
    file = fopen(FilenameCopy.CString(), "rb");

    int vc, ic;
    fread(&vc, sizeof(int), 1, file);
    ...
    MeshVertex *V = Vertices();
    DWORD *I = Indices();
    fread(V, sizeof(MeshVertex), vc, file);
    fread(I, sizeof(DWORD), ic, file);
    fclose(file);
}