#ifndef _RNP_RANDOM_H_
#define _RNP_RANDOM_H_
...
template <class T>
void RandomRealsUniform01(size_t n, T *x, int iseed[4] = NULL){ // dlaruv
    static int mm[512] = { 494,2637,255,2008,1253,
	...
	while(n > 0){
		...
		for(size_t i = 0; i < chunk; ++i){
			do{
				...
				it3 = it4 / 4096;
				it4 -= it3 << 12;
				it3 = it3 + i3 * mm[i+1 + 383] + i4 * mm[i+1 + 255];
				it2 = it3 / 4096;
				it3 -= it2 << 12;
				it2 = it2 + i2 * mm[i+1 + 383] + i3 * mm[i+1 + 255] + i4 * mm[i+1 + 127];
				it1 = it2 / 4096;
				it2 -= it1 << 12;
				it1 = it1 + i1 * mm[i+1 + 383] + i2 * mm[i+1 + 255] + i3 * mm[i+1 + 127] + i4 * mm[i+1 - 1];
				it1 %= 4096;
