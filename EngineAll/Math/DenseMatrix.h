...
public:
    ...
    __forceinline const T& Cell(int Row, int Col) const
    {
        return _Data[Row * _ColCount + Col];
    }
    ...
    bool VictorEigenSystem(Vector<T> &Eigenvalues, DenseMatrix<T> &Eigenvectors) const;
...
template<class T>
bool DenseMatrix<T>::VictorEigenSystem(Vector<T> &Eigenvalues, DenseMatrix<T> &Eigenvectors) const
{
...
String DenseMatrix<T>::EigenTest(const T *Eigenvalues, const T **Eigenvectors) const
{
  ...
  for(UINT EigenIndex = 0; EigenIndex < _RowCount; EigenIndex++)
  {
    ...
    for(UINT ElementIndex = 0; ElementIndex < _RowCount; ElementIndex++)
    {
      Error += Math::Abs(Eigenvector[ElementIndex] * CurEigenvalue - Result[ElementIndex]);
    }
    if(Verbose)
    {
...
template<class T>
DenseMatrix<T>DenseMatrix<T>::Inverse()
{
