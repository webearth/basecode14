...
void UVAtlas::MakeDescriptiveBitmap(Bitmap &Bmp)
{
    ...
    for(UINT y = 0; y < Bmp.Height(); y++)
    {
        ...
        {
            const UVAtlasCell &curCell = _Cells.GetElement(y, x);
            RGBColor &C = Bmp[y][x];
...
void UVAtlas::Create(const BaseMesh &MeshIn, BaseMesh &MeshOut, UINT TextureWidth, UINT TextureHeight, bool UseProvidedTexCoords, const String &FastLoadDirectory)
{
    ...
    for(UINT y = 0; y < TextureHeight; y++)
    {
        ...
        {
            UVAtlasCell &CurCell = _Cells.GetElement(y, x);
            CurCell.BarycentricCoord = BarycentricCoordGrid.GetElement(y, x);
            CurCell.TextelClass = TextelClassGrid.GetElement(y, x);
            CurCell.FaceIndex = FaceIndexGrid.GetElement(y, x);
        }
    }
}
