...
public:
    ...
    void FreeMemory()
    {
        for(UINT RowIndex = 0; RowIndex < _Classifiers.Rows(); RowIndex++)
        {
            ...
            {
                BinaryClassifier<LearnerInput> *CurClassifier = _Classifiers.GetElement(RowIndex, ColIndex);
    ...
    void Train(const Dataset &Examples)
    {
        ...
        for(UINT RowIndex = 0; RowIndex < ClassCount; RowIndex++)
        {
            ...
            {
                _Classifiers.GetElement(RowIndex, ColIndex) = _Config.Factory->MakeClassifier();
                BinaryClassifier<LearnerInput> &CurClassifier = *(_Classifiers.GetElement(RowIndex, ColIndex));
