...
WindowObjects *g_WndProcContext = NULL;
...
LRESULT WINAPI WndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_KEYDOWN:
        ...
        case VK_ESCAPE:
            ...
            break;
        default:
            g_WndProcContext->GetInputManager().SetKeyState(wParam, true);
            break;
        }
        break;

    case WM_KEYUP:
        g_WndProcContext->GetInputManager().SetKeyState(wParam, false);
        break;

    case WM_COMMAND:
        if ( 0 == HIWORD(wParam) )
        {
            g_WndProcContext->GetInputManager().PushEvent(InputEvent(InputEventMenu, wParam));
           //PostQuitMessage(0);
        }
        break;

    case WM_SIZE:
        g_WndProcContext->GetFrameTimer().Pause();
        g_WndProcContext->QueueReSize();
        break;
    ...
    case WM_SYSKEYDOWN:
        ...
        {
            g_WndProcContext->GetInputManager().SetKeyState(KEY_ALT_ENTER, true);
            return NULL;
        }
      break;
      case WM_SYSKEYUP:
        ...
        if ((TCHAR)wParam == KEY_ENTER)
        {
            g_WndProcContext->GetInputManager().SetKeyState(KEY_ALT_ENTER, false);
            return NULL;
        }
        break;

    case WM_LBUTTONDOWN:
        g_WndProcContext->GetInputManager().SetMouseState(MouseButtonLeft, true);
        break;

    case WM_LBUTTONUP:
        g_WndProcContext->GetInputManager().SetMouseState(MouseButtonLeft, false);
        break;

    case WM_RBUTTONDOWN:
        g_WndProcContext->GetInputManager().SetMouseState(MouseButtonRight, true);
        break;

    case WM_RBUTTONUP:
        g_WndProcContext->GetInputManager().SetMouseState(MouseButtonRight, false);
        break;

    case WM_MBUTTONDOWN:
        g_WndProcContext->GetInputManager().SetMouseState(MouseButtonMiddle, true);
        break;

    case WM_MBUTTONUP:
        g_WndProcContext->GetInputManager().SetMouseState(MouseButtonMiddle, false);
        break;
    ...
    case WM_MOUSEMOVE:
        {
            ...
            g_WndProcContext->GetInputManager().UpdateMousePos(NewPos);
        }
        break;

    case WM_MOUSEWHEEL:
        g_WndProcContext->GetInputManager().UpdateWheelState(GET_WHEEL_DELTA_WPARAM(wParam));
        break;
