...
{
    _Shift = S.IsPrefix("Shift+");
    _Ctrl = S.IsPrefix("Ctrl+");
    _Alt = S.IsPrefix("Alt+");
...
Menu& Menu::AddSubMenu(WindowManager &WM, UINT Id, const String &Name)
{
    ...
    Success = DrawMenuBar(WM.CastWindows().GetHWND());
    Assert(Success != 0, "DrawMenuBar failed");

    return _Entries.Last().SubMenu();
}

void Menu::AddSeperator(WindowManager &WM, UINT Id)
{
...
void Menu::AddString(WindowManager &WM, UINT Id, const String &Name)
{
...
void Menu::SetItemName(WindowManager &WM, UINT Id, const String &Name)
{
...
void Menu::SetItemState(WindowManager &WM, UINT Id, bool Enabled, bool Checked)
{
    MENUITEMINFO Info;
    Info.cbSize = sizeof(MENUITEMINFO);
...
void MenuInterface::Init(WindowManager &WM)
{
    HMENU MenuBarHandle = CreateMenu();
    Assert(MenuBarHandle != NULL, "CreateMenu failed");

    BOOL Success = SetMenu(WM.CastWindows().GetHWND(), MenuBarHandle);
    Assert(Success != 0, "SetMenu failed");
