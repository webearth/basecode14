...
{
    float Min = Values.GetElement(0, 0);
    float Max = Values.GetElement(0, 0);
    for(UINT y = 0; y < Values.Rows(); y++)
    {
        for(UINT x = 0; x < Values.Cols(); x++)
        {
            if(Values.GetElement(y, x) != 0.0f)
            {
                if(Min == 0.0f)
                {
                    Min = Values.GetElement(y, x);
                }
                Min = min(Min, Values.GetElement(y, x));
            }
            Max = max(Max, Values.GetElement(y, x));
        }
    }
...
void Bitmap::LoadGrid(const Grid<float> &Values, float Min, float Max)
{
    ...
    for(UINT y = 0; y < _Height; y++)
    {
        for(UINT x = 0; x < _Width; x++)
        {
            ...
            if(Max != Min)
            {
                Intensity = Utility::BoundToByte(Math::LinearMap(Min, Max, 0.0f, 255.0f, Values.GetElement(y, x)));
            }
            if(Values.GetElement(y, x) == 0.0f)
            {
