...
namespace Math
{
    ...
    //
    // squares an arbitrary type of data
    //
    ...
    //
    // Given a function on the "real line", performs a given number of iterations of
    // Newton's method, starting at an initial guess s.  The resulting point is also
    // stores at s.
    //
    void FindZeroNewton(float &s, UINT Iterations, float (*RealFunction) (float));
...
namespace Utility
{
    ...
    //
    #define D3DValidate(x, Function) { HRESULT hr = (x); if(FAILED(hr)) SignalError(String("The function ") + Function + String(" has unexpectedly failed; the program will now abort."));}
    ...
    template <class type> __forceinline UINT Hash32(const type &Obj)
    {
    ...
    //
    void GetFileLines(ifstream &File, Vector<String> &Output);
    void GetFileLines(const String &Filename, Vector<String> &Output);
    ...
    /*__forceinline int QSortCastFloatToInt(float x)
    {
        if(x < 0.0f)
        {
            return 1;
