...
public:
    ...
    // ASCII
    void SaveASCII(const String &Filename);        //saves the mesh in an ASCII format
    void LoadASCII(const String &Filename);        //loads the mesh in an ASCII format

    // Binary
    void SaveBinary(const String &Filename);    //saves the mesh in a binary format
    void LoadBinary(const String &Filename);    //loads the mesh in a binary format
    ...
    void PerfectSplit(Plane &p, BaseMesh &M1, BaseMesh &M2);    //PerfectSplit is the same as split except triangles that lie on the border are split such that the cut is even rather than jagged.
    void PerfectSplit(float (*PositionFunction) (Vec3f &), BaseMesh &M1, BaseMesh &M2);
