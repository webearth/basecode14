...
void BaseMesh::PerfectSplit(Plane &p, BaseMesh &M1, BaseMesh &M2)
{
    BaseMeshPlane = p;
    PerfectSplit(BaseMeshPlaneFunction, M1, M2);
}
...
void BaseMesh::PerfectSplit(float (*PositionFunction) (Vec3f &), BaseMesh &M1, BaseMesh &M2)
{
    int i,i2,vc=VertexCount(),ic=IndexCount();
    MeshVertex *V = Vertices();
    DWORD *I = Indices();
    ...
    PerfectSplitPositionFunction = PositionFunction;
    ...
    for(i=0;i<vc;i++)
    {
        ...
        {
            VMap[i].Side = 0;
            VMap[i].NVMap = NV1.Length();
            NV1.PushEnd(V[i]);
        } else {
            VMap[i].Side = 1;
            VMap[i].NVMap = NV2.Length();
            NV2.PushEnd(V[i]);
        }
    }

    int TSide[3],TriangleType;
    TriMeshFace Tri;

    DWORD LocalTriangleM1[6];
    DWORD LocalTriangleM2[6];

    MeshVertex Vtx1, Vtx2, MiddleVtx;
    float s;

    for(i=0;i<ic;i+=3)
    {
        TSide[0] = VMap[I[i+0]].Side;
        TSide[1] = VMap[I[i+1]].Side;
        TSide[2] = VMap[I[i+2]].Side;

        LocalTriangleM1[0] = VMap[I[i+0]].NVMap;
        LocalTriangleM1[1] = VMap[I[i+1]].NVMap;
        LocalTriangleM1[2] = VMap[I[i+2]].NVMap;

        LocalTriangleM2[0] = VMap[I[i+0]].NVMap;
        LocalTriangleM2[1] = VMap[I[i+1]].NVMap;
        LocalTriangleM2[2] = VMap[I[i+2]].NVMap;

        TriangleType = TSide[0] * 4 + TSide[1] * 2 + TSide[2] * 1;

        for(i2=0;i2<3;i2++)
        {
            if(PerfectEdges[TriangleType][i2])
            {
                Vtx1 = V[I[i+PerfectEdgeList[i2][0]]];
                Vtx2 = V[I[i+PerfectEdgeList[i2][1]]];
                
                PerfectSplitV1 = Vtx1.Pos;
                PerfectSplitV2 = Vtx2.Pos;

                s = 0.5f;
                Math::FindZeroNewton(s, 10, PerfectSplitZeroSearch);
                if(s < 0.0f) s = 0.0f;
                if(s > 1.0f) s = 1.0f;
                ...
                LocalTriangleM2[3+i2] = NV2.Length();
                NV2.PushEnd(MiddleVtx);
            }
        }

        for(i2=0;i2<6;i2+=3)
        {
            if(M1Indices[TriangleType][i2+0] != -1)
            {
                Tri.I[0] = LocalTriangleM1[M1Indices[TriangleType][i2+0]];
                Tri.I[1] = LocalTriangleM1[M1Indices[TriangleType][i2+1]];
                Tri.I[2] = LocalTriangleM1[M1Indices[TriangleType][i2+2]];
                NT1.PushEnd(Tri);
            }
            if(M2Indices[TriangleType][i2+0] != -1)
            {
                Tri.I[0] = LocalTriangleM2[M2Indices[TriangleType][i2+0]];
                Tri.I[1] = LocalTriangleM2[M2Indices[TriangleType][i2+1]];
                Tri.I[2] = LocalTriangleM2[M2Indices[TriangleType][i2+2]];
                NT2.PushEnd(Tri);
            }
        }
    }
