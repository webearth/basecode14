...
struct ApplyElementaryReflector{ // zlarf, dlarf, clarf, slarf
	template <class T>
	ApplyElementaryReflector(size_t m, size_t n, const T *v, int incv, const T &tau, T *c, int ldc, T *work){
		...
		if('L' == side){ // Form  H * C
			...
			if(lenv > 0 && lenc > 0){
				...
				RNP::TBLAS::MultMV<'T'>(lenc, lenv, T(1), c, ldc, v, incv, T(0), work, 1);
...
template <class T> // zlarfg, zlarfp, dlarfg, dlarfp, clarfg, clarfp, slarfg, slarfp
