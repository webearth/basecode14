...
bool QZSorter_Nearest(const std::complex<double> &n1, const std::complex<double> &d1, const std::complex<double> &n2, const std::complex<double> &d2, void *data){
...
bool QZSorter_MaxReal(const std::complex<double> &n1, const std::complex<double> &d1, const std::complex<double> &n2, const std::complex<double> &d2, void *data){
	// If n1 = a+bi, d1 = c+di,
...
void jdqz_linsolve_IDRs(
	...
	RNP::IDRs(n, Op, b, x, data->ls_params, params, data->workspace, op_data, &jdqz_preconditioner, precon_data);
	max_mult = data->ls_params.max_mult;
}

void jdqz_linsolve_GMRES(
	size_t n,
	void (*Op)(const std::complex<double> *, std::complex<double> *, void*),
	std::complex<double> *x,
	std::complex<double> *b,
	...
	data->ls_params.tol = tol;
	RNP::TBLAS::Scale(n, -1., b, 1);
	RNP::GMRES(n, Op, b, x, data->ls_params, params, data->workspace, op_data, &jdqz_preconditioner, precon_data);
	max_mult = data->ls_params.max_mult;
}
...
)
{
	...
	const size_t default_jmin = 2*kmax;
	const size_t default_jmax = (3*kmax >= 20 ? 3*kmax : 20);
	const size_t jmin = ((params.min_search_space == 0) ? default_jmin : params.min_search_space);
	const size_t jmax = ((params.max_search_space == 0) ? default_jmax : params.min_search_space);
	if(jmax <= jmin){ return -11; }
	if(kmax > jmax){ return -11; }

	// Setup the workspace
	std::complex<double> *_work = params.work;
	...
	if(params.work == NULL){
		...
		//   bconv = zeros(jmax,1)
		_work = new std::complex<double>[n*(2+5*jmax+3*kmax)+jmax*(10*jmax+3)];
	...
	double *rwork = new double[8*jmax];
	std::complex<double> *zwork = new std::complex<double>[2*jmax];
	size_t *ipivqkz = new size_t[jmax];
	...
	while(k < kmax && step < params.max_iters){
		...
		if(j == 0){ // set v to initial value v0
			...
			op_data.beta = zbeta;
			params.linear_solver(n, &jdqz_op, &v[0+j*n], rhs, deps, linsolve_max_mult, &op_data, &precon_data, params.linear_solver_data);
		}
