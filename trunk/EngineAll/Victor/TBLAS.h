...
typename _RealOrComplexChooser<T>::real_type Norm2(size_t n, const T *x, size_t incx){
	...
	while(n --> 0){
		...
		/*
		real_type ca(_RealOrComplexChooser<T>::_abs(*x));
		if(scale < ca){
			real_type r = scale/ca;
			ssq = real_type(1) + ssq*r*r;
			scale = ca;
		}else{
	...
	real_type ssq(0);
	while(n --> 0){
		ssq += _RealOrComplexChooser<T>::_abs2(*x);
		x += incx;
	}
	using namespace std;
	return sqrt(ssq);
...
struct MultMM{
	...
	MultMM(size_t m, size_t n, size_t k, const A &alpha, const T *a, size_t lda,
		...
		const size_t nrowa = (nota ? m : k);
		const size_t ncola = (nota ? k : m);
		const size_t nrowb = (notb ? k : n);
